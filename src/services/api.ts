...
api.interceptors.response.use(
  ...
  (error) => {
    const message = error.response?.data?.message || "Ha ocurrido un error. Por favor intente de nuevo."
...
const apiService = {
  ...
  auth: {
    ...
    // Nuevo método para verificar preguntas de seguridad
    verifySecurityQuestions: async (email: string, answer1: string, answer2: string, answer3: string) => {
      try {
    ...
    // Nuevo método para resetear contraseña con token
  ...
  reports: {
    ...
    getAttendanceSummary: async (periodo = "week") => {
      try {
