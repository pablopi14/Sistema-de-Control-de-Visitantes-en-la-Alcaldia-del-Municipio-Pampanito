...
exports.verifySecurityQuestions = async (req, res) => {
  try {
    ...
    const answersAreValid = await user.verifySecurityAnswers(answer1, answer2, answer3)
    ...
    // Generar token temporal para resetear contraseña (válido por 15 minutos)
    const resetToken = jwt.sign({ id: user.id, purpose: "password-reset" }, config.secret, { expiresIn: "15m" })
    ...
    res.status(500).send({
      message: error.message || "Ocurrió un error al verificar las preguntas de seguridad.",
    })
  }
}
...
exports.resetPassword = async (req, res) => {
  try {
    ...
    try {
      ...
      // Verificar que el token sea para resetear contraseña
    ...
    res.status(500).send({
      message: error.message || "Ocurrió un error al restablecer la contraseña.",
    })
  }
}
