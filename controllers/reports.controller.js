...
const fetchReportData = async (startDate, endDate, cedula = null, departamento = null) => {
  // Validar fechas
  ...
  if (cedula) {
    baseFilters.cedula = cedula
  }

  if (departamento && departamento !== "todos") {
    baseFilters.departamento = departamento
  }

  // Obtener total de visitas en el rango seleccionado con filtros
  const totalVisitas = await Attendance.count({
    where: baseFilters,
  })

  // Visitas por día dentro del rango con filtros
  ...
  // Motivos de visita más populares en el rango seleccionado con filtros
  ...
  // Visitantes recurrentes en el rango seleccionado con filtros
  ...
  return {
    ...
    hasData: totalVisitas > 0,
  }
}

exports.getReportData = async (req, res) => {
  try {
    const { startDate, endDate, cedula, departamento } = req.query

    const reportData = await fetchReportData(startDate, endDate, cedula, departamento)
...
exports.generatePDF = async (req, res) => {
  try {
    const { startDate, endDate, cedula, departamento } = req.body

    // Obtener los datos del reporte usando la función auxiliar con filtros
    const reportData = await fetchReportData(startDate, endDate, cedula, departamento)
    ...
    const infoBoxHeight = cedula || departamento ? 110 : 90
    doc.rect(margin, infoBoxY, contentWidth, infoBoxHeight).stroke("#E5E7EB").fillColor("#F9FAFB").fill()
    ...
    let currentInfoY = infoBoxY + 35
    doc
      .font("Helvetica")
      .fontSize(11)
      .text(`Período: ${startDate} al ${endDate}`, margin + 15, currentInfoY)

    currentInfoY += 15
    doc.text(`Generado: ${new Date().toLocaleString("es-ES")}`, margin + 15, currentInfoY)

    currentInfoY += 15
    doc.text(`Total de registros en el período: ${reportData.totalVisitas}`, margin + 15, currentInfoY)

    if (cedula) {
      currentInfoY += 15
      doc.font("Helvetica-Bold").text(`Filtrado por cédula: ${cedula}`, margin + 15, currentInfoY)
    }

    if (departamento) {
      currentInfoY += 15
      doc.font("Helvetica-Bold").text(`Filtrado por departamento: ${departamento}`, margin + 15, currentInfoY)
    }
    ...
    if (!reportData.hasData) {
      ...
      let noDataMessage = `No se encontraron registros de asistencia para el período seleccionado (${startDate} al ${endDate})`

      if (cedula) {
        noDataMessage += ` con la cédula ${cedula}`
      }

      if (departamento) {
        noDataMessage += ` en el departamento de ${departamento}`
      }
      ...
      doc
        ...
        .font("Helvetica")
        .fillColor("#6B7280")
        .text(noDataMessage, margin + 20, doc.y, { width: contentWidth - 40, align: "justify" })
        .moveDown(2)
        .text("Posibles razones:", margin + 20, doc.y)
        .moveDown(0.5)
        .text("• No hubo visitas registradas con estos criterios", margin + 40, doc.y)
        .moveDown(0.3)
        .text("• El rango de fechas seleccionado está fuera del período de registros", margin + 40, doc.y)
        .moveDown(0.3)
        .text("• Los filtros aplicados son muy restrictivos", margin + 40, doc.y)
    ...
    const stats = [
      ...
      ["Visitas hoy:", reportData.visitasHoy.toString()],
      ["Visitas última semana:", reportData.visitasSemana.toString()],
      ["Visitas último mes:", reportData.visitasMes.toString()],
    ]
